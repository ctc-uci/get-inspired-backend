...
router.get('/', async (req, res) => {
  try {
    const computations = await pool.query('SELECT * FROM computation;');
...
router.get('/:computationId', async (req, res) => {
  try {
    const { surveyId } = req.params;
    isNumeric(surveyId);
    const [query, params] = toUnnamed('SELECT * FROM computation WHERE survey_id = :surveyId', {
      surveyId,
    });
...
router.post('/', async (req, res) => {
  try {
    ...
    SELECT * s WHERE id = LAST_INSERT_ID();`,
      columnNames.reduce(
        (acc, current) => ({
          ...acc,
...
router.delete('/:computationId', async (req, res) => {
  try {
    ...
    const [query, params] = toUnnamed(`DELETE from computation WHERE id = :computationId;`, {
      computationId,
    });
    const computation = await pool.query(query, params);
